...
def log_likelihood (time_delays,magnifications,samples_mcmc, y_rel, theta): 
    ...
    x_gw = x_source + theta[0]
    y_gw = y_source + theta[1]
...
def log_prior (theta):
    ...
    if np.abs(x) < 0.5 and np.abs(y) < 0.5:
        lp_x = -0.5 * ((x - mu)/sigma)**2 - np.log(sigma * np.sqrt(2 * np.pi))
        lp_y = -0.5 * ((y - mu)/sigma)**2 - np.log(sigma * np.sqrt(2 * np.pi))
        return lp_x + lp_y
